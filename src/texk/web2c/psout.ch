...
begin 
  ...
  if internal[gtroffmode]>0 then begin
    print_nl("/ScaleFont{dup fmc 0 get Amul 0 exch put dup dup 3 get Amul 3 exch put fmd}bd");
    end;
...
if (applied_reencoding(f))or(fm_font_slant(f)<>0)or(fm_font_extend(f)<>0)or@|
  ...
  if str_vs_str(font_name[f],"zpzdr-reversed")=0 then begin
    ...
    ps_print("/a12[-904 -939]def ");
    ps_print("end def ");
    end;  ps_print("currentdict end");
  print_ln;
  ps_print_defined_name(f);
