...
int do_newer (char *source, char *target) {
#ifndef INLINE_NEWER
...
void prepare_tex(char *texname,char *tmpname) {
  char buffer[MBUF];
  char *tmpfile = NULL;
...
int main(int ac, char **av)
{
  ...
  strncat(maincmd," --parse-first-line --interaction=nonstopmode", (LNAM-strlen(maincmd)));  
  ...
  while (curarg < (ac-1)) {
    ...
    if (ARGUMENT_IS("-help") || ARGUMENT_IS("--help")) {
      ...
      fprintf(stderr,"Usage: %s [-tex[=<program>]|-troff[=<program>]] MPFILE MPXFILE.\n", av[0]);
