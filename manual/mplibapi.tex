...
frontend. It uses `normal' \MP\ interface with I/O to and from files,
...
\NC int            \NC max_in_open      \NC 10    \NC maximum level of \type{input} nesting.
                                                      ignored in \type{non-ini} mode, read from mem file\NC \NR
\NC int            \NC main_memory      \NC 5000  \NC size of the main memory array.
...
\NC int            \NC print_found_names\NC 0     \NC Controls whether asked or found filename as logged\NC \NR
...
\NC int            \NC troff_mode       \NC 0     \NC set this nonzero to initialize `troffmode' \NC \NR
...
\NC mp_editor_cmd  \NC run_editor       \NC NULL  \NC function called after `E' error response\NC \NR
...
linked list of output images (you need to include \type{psout.h} to be
able to actually make use this list of images)
...
This function is executed when a user has pressed `E' as reply to an
...
These are all defined in \type{psout.h}
...
Coordinate values are in scaled points.
...
\item If there is a one-knot pen (\type{pen_p->next} = \type{pen_p});  fill \type{path_p}
...
\item If there is a more complex pen (\type{pen_p->next} != \type{pen_p}), its path 
...
\NC int            \NC height     \NC height etc.\NC \NR
\NC int            \NC depth      \NC depth etc.\NC \NR
...
\starttabulate[|l|l|p|]
\NC struct mp_edge_object *   \NC next      \NC Points to the next figure (or NULL)\NC \NR
\NC struct mp_graphic_object *\NC body      \NC A linked list of objects in this figure \NC \NR
...
\NC int                       \NC width     \NC Value of \type{charwd}. If \MP's \type{fontmaking:=1} was active,
this would be the \TFM\ width (but without the potential rounding correction for \TFM\ file format)\NC \NR
