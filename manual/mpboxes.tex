...
Since the advent of the \texttt{boxes} package several alternative or
competing packages for drawing box diagrams have been developed by the
MetaPost community.  The most widely known ones are
\emph{MetaObj}\index{MetaObj?\emph{MetaObj}},
\emph{MetaUML}\index{MetaUML?\emph{MetaUML}},
\emph{expressg}\index{expressg?\emph{expressg}}, and
\emph{blockdraw\_mp}\index{blockdraw\_mp?\emph{blockdraw\_mp}}.  If you
intend to create lots of technical drawings, you should have a look at
those packages, too.
...
Tables~\ref{pseudotab} and~\ref{ivartab} summarize macros and internal
variables\index{internal variables}\index{variables!internal} provided
by the \texttt{boxes} package.
...
\hline
\hline
\tt \verb|boxit|&    suffix, picture&        --&     \pageref{Dboxit}&
        Define a box containing the picture\\\hline
\tt \verb|boxit|&    suffix, string& --&     \pageref{Dsboxit}&
        Define a box containing text\\\hline
\tt \verb|boxit|&    suffix, \tdescr{empty}& --&     \pageref{Deboxit}&
        Define an empty box\\\hline
\tt \verb|boxjoin|&  equations&      --&     \pageref{Dbxjoin}&
        Give equations for connecting boxes\\\hline
\tt \verb|bpath|&    suffix& path&   \pageref{Dbpath}&
        A box's bounding circle or rectangle\\\hline
\tt \verb|circleit|& suffix, picture&        --&     \pageref{Dcircit}&
        Put picture in a circular box\\\hline
\tt \verb|circleit|& suffix, picture&        --&     \pageref{Dcircit}&
        Put a string in a circular box\\\hline
\tt \verb|circleit|& suffix, \tdescr{empty}& --&     \pageref{Dcircit}&
        Define an empty circular box\\\hline
\tt \verb|drawboxed|&        list of suffixes&       --&     \pageref{Ddrbxed}&
        Draw the named boxes and their\\
&       &       &       &
        contents\\\hline
\tt \verb|drawboxes|&        list of suffixes&       --&     \pageref{Ddrbxes}&
        Draw the named boxes\\\hline
\tt \verb|drawunboxed|&      list of suffixes&       --&     \pageref{Ddrunbx}&
        Draw contents of named boxes\\\hline
\tt \verb|fixpos|&   list of suffixes&       --&     \pageref{Dfixpos}&
        Solve for the size and position of the\\
&       &       &       &
        named boxes\\\hline
\tt \verb|fixsize|&  list of suffixes&       --&     \pageref{Dfixsiz}&
        Solve for size of named boxes\\\hline
\tt \verb|pic|&      suffix& picture&        \pageref{Dpic}&
