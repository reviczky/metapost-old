...
\newcommand*{\regmark}{$^{\mbox{\textregistered}}$}% Registered trademark
...
\begin{document}
\VerbatimFootnotes
%\title{\scalebox{2.04}{\MP}\\[-1ex]\textsc{A USER'S MANUAL}\\[2ex]}
\title{\mbox{}\vfill\scalebox{3}{\MP}\\[0.5ex]\scalebox{1.47}{\textsc{A USER'S MANUAL}}\\[2ex]}
\author{John D. Hobby\\\small and the MetaPost development team\\[2ex]}
\date{documented version: 1.000\\\small\svnToday\vfill\vfill}
\maketitle
...
components the color can be inspected.  Depending on the color model of
...
The color model of a picture component can be requested by the
\ttt{colormodel}\index{colormodel?\texttt{colormodel}}\label{Dcolormodel}
operator (see section~\ref{adv.gr}).  Applying a non-matching color part
