...
section~\ref{Dprologues} for more information on \ttt{prologues}.


\subsection{Using MetaPost graphics in \TeX, \LaTeX,  pdf\LaTeX\ and troff}
\label{Dteximport}
...
\paragraph{pdf\LaTeX}
In contrast to \ttt{dvips}, pdf\LaTeX\ in PDF mode can't process
...
For the first and conventional approach we have to add the line
...
Note, there is an additional difference between \LaTeX\ and pdf\LaTeX.
The application pdf\LaTeX\ is both, a \LaTeX\ interpreter and an output
driver for the PDF document format.  For that reason graphic files
\emph{are} included in a pdf\LaTeX\ run.
