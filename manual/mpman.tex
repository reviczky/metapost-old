...
\end{verbatim}
$$
prints `{\tt mp = 0.99}'.  (Incidentally, \texttt{>} does a simple ASCII
...
The color\index{color type} type is a lot like the pair type, except
...
constants {\tt black}\index{black?\texttt{black}}\label{Dblack}, {\tt
white}\index{white?\texttt{white}}\label{Dwhite}, {\tt
red}\index{red?\texttt{red}}\label{Dred}, {\tt
green}\index{green?\texttt{green}}\label{Dgreen}, {\tt
blue}\index{blue?\texttt{blue}}\label{Dblue}, or the red, green, and
...
PostScript\index{PostScript} output file.  MetaPost solves linear
equations involving colors the same way it does for pairs.

The cmykcolor\index{cmykcolor type} type is a lot like the color type,
...
numeric}\index{numeric?\texttt{numeric}}\label{Dnumop}, {\tt
boolean}\index{boolean?\texttt{boolean}}\label{Dboolop}, {\tt
color}\index{color?\texttt{color}}\label{Dcolrop}, {\tt
string}\index{string?\texttt{string}}\label{Dstrgop}, {\tt
transform}\index{transform?\texttt{transform}}\label{Dtrnfop}, {\tt
path}\index{path?\texttt{path}}\label{Dpathop}, {\tt
pen}\index{pen?\texttt{pen}}\label{Dpenop}, and {\tt
picture}\index{picture?\texttt{picture}}\label{Dpictop}.  Besides just
...
MetaPost does not use the
ligatures\index{ligatures} and kerning\index{kerning} information that
comes with a \TeX\ font.
...
troff\index{troff} instead of \TeX.  When using this option, it is a
good idea to start your MetaPost input file with the assignment {\tt
prologues:=1}\index{prologues?\texttt{prologues}}. 
...
Courier\index{Courier} or Times-Roman\index{Times-Roman}.

\item If you set \ttt{prologues} to~1, the MetaPost
output files will be so-called ``structured
PostScript''\index{PostScript!structured} (EPSF\index{EPSF}), generated
...
%TH rewrite the next par
\item If you set \ttt{prologues} to~2, the MetaPost
output files will also be so-called ``structured
PostScript''\index{PostScript!structured} (EPSF\index{EPSF}), generated
on the assumption that text comes from PostScript fonts provided by the
``environment'', that is, the document viewer or embedded application
using the output.

\item If you set \ttt{prologues} to~3, the MetaPost
output files will include any PostScript fonts it nows about from
reading the font map file. Assuming the map file is complete and 
the font files are  available, the output is completely selfcontained.
\end{itemize}

The \ttt{prologues} variable is irrelevant when processing MetaPost
files through the \ttindex{mptopdf} utility (part of the \ConTeXt\
distribution), because PDF files are by nature stand-alone.

The details on how to include PostScript figures in a paper done in
\TeX\ or troff are system-dependent.  They can generally be found in
manual pages and other on-line documentation.  The manual for the
...
$\tt \qquad \;|\; withrgbcolor \descr{rgb color expression} \;|\;
        withcmykcolor \descr{cmyk color expression}$\\
$\tt \qquad \;|\; withgreyscale \descr{numeric} \;|\;
        withoutcolor $\\
$\tt \qquad \;|\; withprescript \descr{string} \;|\;
        withpostscript \desc{string} $\\
...
\tt     defaultcolormodel&    --& %TH fix this! not numeric
        the color model that is implied when metapost starts:
        "rgb", "cmyk", "grey", or "none" (default "rgb")\\\hline
...
\tt     mpprocset&  --&
        Set to 1 for the inclusion of a metapost procset in the output file, 
        potentially reducing its size\\\hline
\tt     pausing&        --&
...
\tt     troffmode&  --& %TH test 
        Can be used to check whether `-troff' was given as a commandline option\\\hline
...
% Updated 2005--2006 by Taco Hoekwater, Karl Berry, and others.
