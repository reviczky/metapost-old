...
\ttt{picture} variable and finally restore \ttt{currentpicture} to
the saved state.  MetaPost version~0.60 introduced a new macro
$$ \hbox{\tt image($\descr{drawing commands}$)} $$%
\index{image?\texttt{image}}\label{Dimage}%
that eases this task.  It takes as input a sequence of arbitrary
drawing operations and returns a picture variable containing the
...
been defined to save the output of two \ttt{draw} operations as follows:
\begin{figure}[b]
$$ \includegraphics{manfig-55} $$
\caption{Using the \ttt{image} operator to copy objects.}
\label{fig55}
\end{figure}
$$\begin{verbatim}
picture wheel;
...
\end{verbatim}
$$
Another object \ttt{car} re-uses \ttt{wheel} to construct a car.  The
three cars are drawn with two different slantings, finally.
...
\tt \verb|&|&   string& string& string& \pageref{Damp}&
        Concatenation---works for paths $l\hbox{\tt\&}r$\\
&       path&   path&   path&   &
        if $r$ starts exactly where the $l$ ends\\\hline
...
\tt \verb|ASCII|\index{ASCII?\texttt{ASCII}}%
&       --&     string& numeric&        --&
...
\pl\tt \verb|bounded|&     --&     any&    boolean&        \pageref{Dbounded}&
        Is argument a picture with\\
&       &       &       &       & a bounding box?\\\hline
...
\pl\tt \verb|clipped|&     --&     any&    boolean&        \pageref{Dclipped}&
        Is argument a clipped picture?\\\hline
...
\tt \verb|colormodel|&       --&     image object&    numeric&        --&
        What is the color model of the\\
&       &       &       &       &
        image object?\\\hline
...
\tt \verb|dashpart|&    --&     picture&    picture&       \pageref{Ddashpart}&
        Dash pattern of a path in a\\
&       &       &       &       &
        stroked picture\\\hline
...
\tt \verb|time of|&     &       &       &       &
        direction\\\hline
\pl\tt \verb|div|\index{div?\texttt{div}}&      numeric&        numeric&        numeric&        --&
        Integer division $\lfloor l/r\rfloor$\\\hline
\pl\tt \verb|dotprod|&  pair&   pair&   numeric&        \pageref{Ddprod}&
        vector dot product\\\hline
\pl\tt \verb|filled|&     --&     any&    boolean&        \pageref{Dfilled}&
        Is argument a filled outline?\\\hline
\tt \verb|floor|&       --&     numeric&        numeric&        \pageref{Dfloor}&
        Greatest integer less than or equal to\\\hline
\tt \verb|fontpart|&    --&     picture&    string&       \pageref{Dfontpart}&
        Font of a textual picture component\\\hline
...
\tt \verb|hex|\index{hex?\texttt{hex}}& --&     string& numeric&        --&
...
\tt \verb|length|&      --&     path&   numeric&        \pageref{Dlength}&
        Number of components (arcs,\\
&       &     string&    &       \pageref{DlengthString}&
        characters, \ldots) in the argument\\
&       &     picture&   &       \pageref{DlengthPicture}&\\\hline
...
\tt \verb|mexp|\index{mexp?\texttt{mexp}}&        --&     numeric&        numeric&        --&
        The function $\exp(x/256)$\\\hline
\tt \verb|mlog|\index{mlog?\texttt{mlog}}&        --&     numeric&        numeric&        --&
        The function $256\ln(x)$\\\hline
\pl\tt \verb|mod|\index{mod?\texttt{mod}}&      --&     numeric&        numeric&        --&
        The remainder function $l-r\lfloor l/r\rfloor$\\\hline
\tt \verb|normal-|\index{normaldeviate?\texttt{normaldeviate}}&     --&     --&     numeric&        --&
...
\tt \verb|oct|\index{oct?\texttt{oct}}& --&     string& numeric&        --&
        Interpret string as octal number\\\hline
\tt \verb|odd|\index{odd?\texttt{odd}}& --&     numeric&        boolean&        --&
...
\tt \verb|pathpart|&    --&     picture&    path&       \pageref{Dpathpart}&
        Path of a stroked picture component\\\hline
\tt \verb|pen|& --&     any&    boolean&        \pageref{Dpenop}&
        Is the expression of type pen?\\\hline
\tt \verb|penoffset|\index{penoffset?\texttt{penoffset}}&   pair&   pen&    pair&   --&
        Point on the pen furthest to the\\
\tt \verb|of|&  &       &       &       &
        right of the given direction\\\hline
\tt \verb|penpart|& --&     picture&    pen&        \pageref{Dpenpart}&
        Pen of a stroked picture component\\\hline
...
\tt \verb|postcontrol|\index{postcontrol?\texttt{postcontrol}}& numeric&        path&   pair&   --&
        First B\'ezier control point on path\\
\tt \verb|of|&  &       &       &       &
        segment starting at the given time\\\hline
\tt \verb|precontrol|\index{precontrol?\texttt{precontrol}}&  numeric&        path&   pair&   --&
        Last B\'ezier control point on path\\
\tt \verb|of|&  &       &       &       &
        segment ending at the given time\\\hline
\pl\tt \verb|readfrom|&    --&    string&  string&        \pageref{Dreadfrom}&
        Read a line from file\\\hline
...
\pl\tt \verb|stroked|&     --&     any&    boolean&        \pageref{Dstroked}&
        Is argument a stroked line?\\\hline
...
\tt \verb|of|&  &       &       &       &
        \\\hline
\tt \verb|textpart|&    --&     picture&    string&       \pageref{Dtextpart}&
        Text of a textual picture component\\\hline
\pl\tt \verb|textual|&     --&     any&    boolean&        \pageref{Dtextual}&
        Is argument typeset text?\\\hline
...
&       &       pen&    &       &
        \\\hline
\tt \verb|uniform-|\index{uniformdeviate?\texttt{uniformdeviate}}&    --&     numeric&        numeric&        --&
...
\tt \verb|withpost-|& --&     string&     ---&        \pageref{Dwithpost}&
        End raw PostScript code\\
\tt \verb|script|&       &       &    &       &\\\hline
\tt \verb|withpre-|& --&     string&     ---&        \pageref{Dwithpre}&
        Begin raw PostScript code\\
\tt \verb|script|&       &       &    &       &\\\hline
...
\pl\tt \verb|write to|&         \pageref{Dwrite}&
