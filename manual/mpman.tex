...
The type `rgbcolor' is an alias of type `color'.

The cmykcolor\index{cmykcolor type} type is a lot like the color type,
except that it has four components instead of three.  Like colors,
cmykcolors can be added, subtracted, used in mediation expressions, or
multiplied or divided by numerics.  Colors can be specified by giving
the cyan, magenta, yellow, and black components explicitly.  There is
no restriction against cmykcolors ``blacker than black'' or ``whiter than
white'' except all components are snapped back to the $[0,1]$ range
when a cmykcolor is given in a PostScript\index{PostScript} output file.
MetaPost solves linear equations involving cmykcolors the same way it does
for colors. 
...
generated on the assumption that text comes from PostScript fonts
provided by the ``environment'', that is, the document viewer or
embedded application using the output. The output file will attempt to
reencode the font correctly, based on \ttt{fontmapfile} and
\ttt{fontmapline} commands.

\item If you set \ttt{prologues} to~3, the MetaPost
output files will be so-called ``structured
PostScript''\index{PostScript!structured} (EPSF\index{EPSF}) also, but
the output will also contain the actual PostScript font(s) used or a
subset of it,, based on \ttt{fontmapfile} and
\ttt{fontmapline} commands.

\item If you set \ttt{prologues} to~1, the MetaPost output claims to be
EPSF, but it is not really conformant. This variant is kept for backward
compatibility mode with old (troff) documents, but use is deprecated: 
For historical reasons, MetaPost sets  {\tt prologues:=1} when the {\tt
-troff} option is given on the commandline.
...
If there are no \ttt{fontmapfile} statements at all, MetaPost will try
to find a default map file. In order, it will attempt to read {\tt
mpost.map} and {\tt pdftex.map}.

Note: if \ttt{prologues} is set to~1, MetaPost attempts to read a file
called {\tt psfonts.map}, regardless of any {\tt fontmapfile}
