...
PostScript\index{PostScript!structured} (EPSF\index{EPSF}).  \TeX\
users can import EPS graphics by first loading package
...
Actually, graphic files are never included in a
\TeX\index{TeX?\TeX!importing MetaPost files} or
\LaTeX\index{LaTeX?\LaTeX!importing MetaPost files} run.  Instead,
\TeX\ and \LaTeX\ only reserve as much space on a page as the graphic
...
run of the output driver, which in most cases should be
\ttindex{dvips}.

When pdf\LaTeX\index{pdfLaTeX?pdf\LaTeX!importing MetaPost files} is
used in PDF mode, an additional line
$$ \verb+\DeclareGraphicsRule{*}{mps}{*}{}+$$%
\index{DeclareGraphicsRule?\texttt{\textbackslash DeclareGraphicsRule}}%
has to be added to the document preamble after loading package
\ttt{graphicx}.  In contrast to \ttt{dvips}, pdf\LaTeX\ can't process
...
\emph{is} purified EPS, so we are in luck.  The above mentioned command
sets up pdf\LaTeX\ so that it assumes all unknown file extensions as
being purified EPS files.  Refer to the documentation of the
\ttt{graphicx} package for more details.

Alternatively---and this is the recommended way since MetaPost
version~1.000---the primitive \verb+filenametemplate+%
\index{filenametemplate?\texttt{filenametemplate}} can be used to set
the file extension of MetaPost output to
\ttt{.mps}\index{mps?\texttt{mps}}.  See section~2.1 for more
information on \ttt{filenametemplate}.  \ttt{Mps} is pdf\LaTeX's
default extension for purified EPS\index{PostScript!purified} files.
If your're only using pdf\LaTeX\ the \ttt{.mps} file extension can as
well be omitted in the \verb+\includegraphics+ call.  However, if you
want to keep flexibile and be able to compile your documents with both,
\LaTeX\ and pdf\LaTeX\, the \ttt{.mps} extension should not be omitted,
since \LaTeX\ isn't aware of that extension by default.

Note, there is an additional difference between using
\ttt{tex} or \ttt{latex} and \ttt{pdftex} or \ttt{pdflatex}.
Pdf(La)\TeX\ is both, a (La)\TeX\ interpreter and an output driver for
the PDF document format.  For that reason graphic files \emph{are}
finally included in a pdf(La)\LaTeX\ run.
...
MetaPost files can be previewed with any decent PostScript viewer,
e.\,g., GSview\index{GSview}.

The situation becomes a little bit fussy when MetaPost output contains
text.  By default, MetaPost doesn't produce self-contained EPS files,
e.\,g., font resources and encoding vectors are not stored in the
output.  For that reason MetaPost output containing text may be
...
reliably be previewed, may they contain text\footnote{As long as you're
not using \MF\index{metafont?\MF} fonts inside MetaPost.} or not.  The
...
section~7.1 for more information on \ttt{prologues}.
